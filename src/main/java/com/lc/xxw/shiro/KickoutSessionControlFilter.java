 ...
 * @author: xuexiaowei
 * @create: 2019-07-12 09:09
 */

public class KickoutSessionControlFilter extends AccessControlFilter {

    protected final static Logger logger = Logger.getLogger(KickoutSessionControlFilter.class);
    //踢出状态，true标示踢出
    ...
    protected boolean isAccessAllowed(ServletRequest request,
        ...
        String jedisSessionId = redisUtils.get(userName);

        //如果已经包含当前Session，并且是同一个用户，跳过。
        if(null!=jedisSessionId && jedisSessionId.equals((String)sessionId)){
            //更新存储到缓存30分钟（这个时间最好和session的有效期一致或者大于session的有效期）
            //JedisUtils.setex(userName,3600, (String)sessionId);
            redisUtils.setForTimeCustom(userName,(String)sessionId,30, TimeUnit.MINUTES);
            return Boolean.TRUE;
        }
        ...
        if(null!=jedisSessionId && !jedisSessionId.equals((String)sessionId)){
            ...
            //存储到缓存30分钟（这个时间最好和session的有效期一致或者大于session的有效期）
            //JedisUtils.setex(userName, 3600, (String)sessionId);
            redisUtils.setForTimeCustom(userName,(String)sessionId,30, TimeUnit.MINUTES);
            return  Boolean.TRUE;
        }
